...
class My_Tk():
    def __init__(self,sql):
        ...
        mainloop()
    def create(self):
        Label(self.query_frame, text="指令").grid(row=0,sticky='nw')
        
        self.combo=Combobox(self.query_frame, 
                values=[
        ...
        self.combo.grid(row=0,sticky='nw')
        Button(self.query_frame,text='確定',command=self.comfirm).grid(row=1,sticky='nw')
        E1=Entry(self.query_frame,width=100)
        E1.grid(row=2,sticky='nw')
        self.entry=E1
        self.display_label=Label(self.query_frame, text="")
        self.display_label.grid(row=3,sticky='nw') 
        Button(self.query_frame,text='執行',command=self.run).grid(row=4,sticky='nw')
    ...
    def comfirm(self):
        ...
        elif(self.combo.get()=="IN"):
            self.entry.insert(0,"select (attribute) from (table) where (condition) in (select (attribute) from (table) where (condition))")
        elif(self.combo.get()=="NOT IN"):
            self.entry.insert(0,"select (attribute) from (table) where (condition) not in (select (attribute) from (table) where (condition))")
        elif(self.combo.get()=="EXISTS"):
            self.entry.insert(0,"select (attribute) from (table) where (condition) exists (select (attribute) from (table) where (condition))")
        elif(self.combo.get()=="NOT EXISTS"):
            self.entry.insert(0,"select (attribute) from (table) where (condition) not exists (select (attribute) from (table) where (condition))")
        elif(self.combo.get()=="COUNT"):
            self.entry.insert(0,"select count(attribute) from (table) where (condition)")
        elif(self.combo.get()=="SUM"):
            self.entry.insert(0,"select sum(attribute) from (table) where (condition)")
        elif(self.combo.get()=="MAX"):
            self.entry.insert(0,"select max(attribute) from (table) where (condition)")
        elif(self.combo.get()=="MIN"):
            self.entry.insert(0,"select min(attribute) from (table) where (condition)")
        elif(self.combo.get()=="AVG"):
            self.entry.insert(0,"select avg(attribute) from (table) where (condition)")
        elif(self.combo.get()=="HAVING"):
            self.entry.insert(0,"select (attribute) from (table) grouby (attribute) having (condition)")
    def run(self):
        self.display_label.config(text=self.entry.get())
        if(self.combo.get()=="SELECT-FROM-WHERE" or self.combo.get()=="IN"or self.combo.get()=="NOT IN"
           or self.combo.get()=="EXISTS"or self.combo.get()=="NOT EXISTS" 
           or self.combo.get()=="COUNT"or self.combo.get()=="SUM"
           or self.combo.get()=="MAX"or self.combo.get()=="MIN"
           or self.combo.get()=="AVG"or self.combo.get()=="HAVING"):
            try:
...
if __name__=="__main__":
  ...
  sql.create_table("book","book_name varchar(20) primary key,id varchar(20) default 'none'\
                   ,borrow_time varchar(20) default 'none',price int,different varchar(20)")
